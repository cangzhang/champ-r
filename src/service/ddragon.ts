...
export const getChampions = async (ver: string, region = 'en_US') => {
  const { data }: { data: { [key: string]: IChampionInfo } } = await http.get(
    `${DDragonCDNUrl}/${ver}/data/${region}/champion.json`,
  );
  return data;
};
...
export const getItemIcon = (id: string | number, ver: string) =>
  `${DDragonCDNUrl}/${ver}/img/item/${id}.png`;
